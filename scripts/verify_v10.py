...
def verify_v10_near_native():
    ...
    academic_markers = {"哲学", "政治", "经济", "体系", "范畴", "逻辑", "理论", "机制", "策略", "规律"}
    ...
    for i, q in enumerate(test_engine.questions):
        ...
        is_academic = any(k in target_word.meaning.lower() or k in target_word.hanzi for k in academic_markers) if target_word else False
        ...
        if target_word:
            target_keywords = set(target_word.meaning.lower().replace(";", "").replace(",", "").split())
            semantic_traps = []
            for opt in q.options:
                if opt == q.correct_answer: continue
                opt_word = next((w for w in test_engine.words if w.hanzi == opt), None)
                if opt_word:
                    opt_keywords = set(opt_word.meaning.lower().replace(";", "").replace(",", "").split())
