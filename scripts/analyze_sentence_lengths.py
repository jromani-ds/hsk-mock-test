...
def analyze_lengths(file_path):
    ...
    for word_entry in vocab_list:
        for sent in word_entry.get("sentences", []):
            text = sent.replace(
                " ", ""
            )  # Remove spaces if any (though Chinese usually doesn't have them in raw source, Tatoeba might)
    ...
    print(
        f"Sentences < {THRESHOLD} chars: {len(short_sentences)} ({len(short_sentences) / len(lengths) * 100:.2f}%)"
    )
