...
def audit_coverage():
    ...
    for level in range(1, 10):
        ...
        print(f"Level {level}: {words_with_sentences}/{total_words} words have sentences ({words_with_sentences/total_words*100:.1f}%)")
        ...
        if level >= 7:
            ...
            print(f"  -> Single Char with Sentences: {single_w_sent}/{total_single} ({single_w_sent/total_single*100:.1f}%)")
            print(f"  -> Compounds with Sentences:   {compound_w_sent}/{total_compound} ({compound_w_sent/total_compound*100:.1f}%)")
