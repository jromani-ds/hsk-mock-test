...
def audit_level(level: int):
    ...
    for q in questions:
        ...
        print(f"Target: {target_hanzi} (POS: {target_word.pos if target_word else '?'}, Level: {target_word.level if target_word else '?'})")
        ...
        if match_pos: pos_parallelism += 1
        if match_len: len_parallelism += 1
