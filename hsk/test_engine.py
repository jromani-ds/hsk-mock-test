...
from hsk.constants import QUESTION_TYPE_MC, QUESTION_TYPE_FIB, QUESTION_TYPE_WRITING, PASSING_SCORE_PERCENTAGE
...
class HSKTestEngine:
    ...
    def __init__(self, level: int, data_engine: DataEngine, num_questions: int = 10):
        ...
        if self.level >= 7:
            for l in range(7, 10):
                self.data_engine.load_level_data(l)
        else:
        ...
        if self.level >= 7:
            word_map = {}
            for l in range(7, 10):
                for w in self.data_engine.get_words_for_level(l):
                    word_map[w.hanzi] = w
    ...
    def _generate_test(self, num_questions: int):
        ...
        if self.level >= 7:
            ...
            academic_keywords = {"哲学", "政治", "经济", "体系", "范畴", "逻辑", "理论", "机制", "策略", "规律", "固然", "诚然"}
            ...
            filtered_pool.sort(key=lambda w: (
                len(w.sentences) > 0,   
    ...
    def _create_writing_question(self) -> Optional[Question]:
        ...
        if self.level == 5:
            ...
            return Question(
                id="WRITING_L5",
                prompt=f"Writing Task (写作): Write a short paragraph (~80 characters) using these words:\n{words_str}",
        ...
        # Since we don't have articles to summarize, we fallback to Narrative Writing based on a Topic Word
        elif self.level == 6:
            if not self.words: return None
            topic_word = random.choice(self.words)
            return Question(
                id="WRITING_L6",
                prompt=f"Writing Task (写作): Write a narrative essay (~400 characters) surrounding the theme: 「{topic_word.hanzi}」",
                type=QUESTION_TYPE_WRITING,
                correct_answer="[Self-Graded]",
                options=[],
        ...
        # Level 7-9: Argument/Thesis (600+ chars)
            ...
            return Question(
                id="WRITING_L79",
                prompt=f"Writing Task (写作): Write an argumentative thesis (~600 characters) analyzing: 「{topic_word.hanzi}」",
                type=QUESTION_TYPE_WRITING,
                correct_answer="[Self-Graded]",
                options=[],
    ...
    def _create_question_for_word(self, word: Word) -> Question:
        ...
        if word.sentences:
            ...
            rhetorical_markers = ["与其", "毋宁", "甚至", "即便", "既然", "不仅", "岂", "何必", "固然", "何况", "所谓", "诚然"]
            
            # REGISTER TRIGGER KEYWORDS (For Discrimination)
            register_triggers = ["政治", "理论", "学术", "机构", "规则", "逻辑", "范畴", "哲学", "利益", "关系"]

            # FACTOID BLACKLIST (Biology, Chemistry, Basic Physics)
            factoid_keywords = {"二氧化碳", "氧气", "光合作用", "肺", "太阳系", "原子", "分子", "科学发现", "排出", "吸收"}
    ...
    def _get_t3_distractors(self, target: Word, candidates: List[Word], count: int) -> List[str]:
        ...
        candidates = [w for w in self.words if w.hanzi != target.hanzi]
        if target.pos:
            target_pos_set = set(target.pos)
            pos_matches = [w for w in candidates if w.pos and set(w.pos).intersection(target_pos_set)]
            if len(pos_matches) >= count:
                candidates = pos_matches
                
        if len(candidates) < count:
            return ["Error1", "Error2", "Error3"][:count] if use_hanzi else [f"Meaning{i}" for i in range(count)]
            
        selected = random.sample(candidates, count)
        return [w.hanzi if use_hanzi else w.meaning for w in selected]
